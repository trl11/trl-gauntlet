"""The acquisition module, as this suite reaches it.

Gauntlet owns the device. A suite naming ``daq`` in ``requires:`` is granted a
URL and drives it over HTTP, which is why nothing here loads NI's driver or
knows which module is fitted: the chassis, the module and the gRPC server the
driver may be reached through are all on the far side of that URL.
...
class Daq:
    ...
    def sample(self) -> dict[str, Any]:
        """Acquire once and return every channel as the module reports it.

        An acquisition rather than a read of the last one: reading would answer
        from whatever the panel last refreshed, which at a slow cadence is a
        value older than the sample it is being recorded as. Each reading is
        the mean of that acquisition, which is what a delta-sigma converter
        gives in place of a single conversion.
        """
