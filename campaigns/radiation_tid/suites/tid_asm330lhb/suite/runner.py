...
Component      ASM330LHBG1TR
Test vehicle   STEVAL-MKI244A
Host           Linux laptop through the eval MCU and STEVAL-MKI109D
Fixture        4-1

To implement:
  - Execute a continuous capture loop monitoring accelerometer and gyroscope outputs.

Hardware       Waiting on STEVAL-MKI244A.
Test plan edits outstanding:
  - Add a note indicating what the orange block is.
  - Remove 12V from the diagram.
  - Remove voltage monitoring for the STEVAL-MKI244A.
  - Step down from 34V to 5V in the diagram.
"""
...
class TidAsm330lhbProfile(BaseModel):
...
def _evaluate(outcomes: list[IterationOutcome], profile: TidAsm330lhbProfile) -> tuple[bool, str] | None:
...
SPEC = SuiteSpec(
    name="tid_asm330lhb",
    profile_model=TidAsm330lhbProfile,
    iterate=_iterate,
    evaluate=_evaluate,
