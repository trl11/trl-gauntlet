"""Watch the GMSL link while the MAX96792AGTM/VY+ is under the beam.

The deserializer, in the USB adapter, is the part being irradiated. The serializer sits
in the camera head outside the beam and is the known-good reference: one capture
...
from suite.profile import TidMax96792Profile
...
def _evaluate(outcomes: list[IterationOutcome], profile: TidMax96792Profile) -> tuple[bool, str] | None:
...
def _hardware(ctx: SuiteContext, profile: TidMax96792Profile) -> dict[str, dict[str, str]]:
    ...
    return {
        "camera": {
            ...
            "part_under_beam": "MAX96792AGTM/VY+",
        }
    }


def _profile_summary(ctx: SuiteContext, profile: TidMax96792Profile) -> dict[str, str]:
    return {
        "driver": profile.driver,
...
SPEC = SuiteSpec(
    name="tid_max96792",
    profile_model=TidMax96792Profile,
    setup=_setup,
    iterate=_iterate,
    evaluate=_evaluate,
