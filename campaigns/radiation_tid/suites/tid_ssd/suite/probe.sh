...
# smartctl lives in /usr/sbin, which a non-interactive SSH session for a
# non-root user does not have on PATH. Without this the probe reports the
# drive as having no SMART counters on a rig where it plainly has.
...
# node is a partition, the node itself when it is not.
disk_of() {
  parent=$(lsblk -no PKNAME "$1" 2>/dev/null | head -n 1)
  if [ -n "$parent" ]; then
    printf '%s\n' "$parent"
  else
    lsblk -no KNAME "$1" 2>/dev/null | head -n 1
  fi
}

# The filesystem written to must sit on the drive being measured. A TEST_PATH
...
if [ "$device_present" = true ]; then
  ...
  if [ -z "$test_path_disk" ] || [ "$test_path_disk" != "$device_disk" ]; then
    test_path_ok=false
    err="${err}test_path_not_on_device;"
  fi
fi
