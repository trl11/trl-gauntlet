...
```bash
pip install gauntlet-suite
```

## Usage

```python
from gauntlet_suite import IterationOutcome, SuiteSpec, make_suite_cli
...
```python
from gauntlet_suite import run_environment, write_simple_verdict
...
|---|---|
| `gauntlet_suite.contract` | Pydantic models for every file crossing the boundary |
| `gauntlet_suite.runner` | `SuiteSpec` and `run_suite` |
| `gauntlet_suite.iteration` | The loop, outcomes, and results |
| `gauntlet_suite.environment` | Reading the contract environment |
| `gauntlet_suite.profile` | Profile loading and validation |
| `gauntlet_suite.reporting` | Artifact writers, usable standalone |
| `gauntlet_suite.phases` | Timing named steps within an iteration |
| `gauntlet_suite.remote` | SSH access to a unit under test (`remote` extra) |
| `gauntlet_suite.monitor` | Background sampling of a remote host |
| `gauntlet_suite.anomalies` | Anomaly recording with running counts |
