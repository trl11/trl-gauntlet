...
class TestProfileDuplicate:
    def test_copies_into_the_user_directory(self, client) -> None:
        response = client.post("/api/suites/alpha/profiles/quick.yaml/duplicate", json={"name": "mine"})
    ...
    def test_a_taken_name_is_409(self, client) -> None:
        client.post("/api/suites/alpha/profiles/quick.yaml/duplicate", json={"name": "mine"})
        response = client.post("/api/suites/alpha/profiles/quick.yaml/duplicate", json={"name": "mine"})
        assert response.status_code == 409

    def test_the_suite_own_name_is_409(self, client) -> None:
        response = client.post("/api/suites/alpha/profiles/quick.yaml/duplicate", json={"name": "quick"})
        assert response.status_code == 409

    def test_a_path_as_a_name_is_422(self, client) -> None:
        response = client.post("/api/suites/alpha/profiles/quick.yaml/duplicate", json={"name": "../escape"})
...
class TestProfileBodies:
    ...
    def test_a_duplicate_without_a_name_is_422(self, client) -> None:
        assert client.post("/api/suites/alpha/profiles/quick.yaml/duplicate", json={}).status_code == 422
...
class TestProfileDelete:
    ...
    def test_a_suite_own_profile_is_409(self, client) -> None:
        assert client.delete("/api/suites/alpha/profiles/quick.yaml").status_code == 409
        assert client.get("/api/suites/alpha/profiles/quick.yaml").status_code == 200
